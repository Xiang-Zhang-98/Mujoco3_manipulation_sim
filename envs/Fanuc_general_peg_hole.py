...
class Fanuc_peg_in_hole(gym.Env):
    ...
    def get_RL_obs(self):
        ...
        eef_eul = trans_eul.mat2euler(eef_rotm.T)
        ...
        state = np.concatenate([100*eef_rotm.T@eef_pos, eef_eul, np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @eef_vel, 
                                np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @world_force])
    ...
    def admittance_control(self, ctl_ori=True):
        ## Get robot motion from desired dynamics (world frame)
        ...
        # From now, we solve everything in body frame
        body_force = np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @ world_force
        world_pos_error = eef_pos - eef_pos_d
        body_stiff_pos_fb = eef_rotm.T @ eef_rotm_d @ np.diag(self.adm_kp[:3]) @ eef_rotm_d.T @ world_pos_error
        body_stiff_ori_fb = self.vee_map(np.diag(self.adm_kp[3:])@eef_rotm_d.T@eef_rotm - eef_rotm.T@eef_rotm_d@np.diag(self.adm_kp[3:]))
        body_stiff_fb = np.concatenate([body_stiff_pos_fb, body_stiff_ori_fb])
        body_damp_fb = np.diag(self.adm_kd)@ np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @(eef_vel - eef_vel_d)
        MA = body_force - body_stiff_fb - body_damp_fb
        # dynamics
        adm_acc = np.divide(MA, self.adm_m)
        T = 1 / self.HZ
        adm_vel = np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @ eef_vel + adm_acc * T # This vel is for eef not link6, which we can control. Also it's in the body frame
        link6_pos, link6_rotm, link6_vel = self.get_link6_pose_vel_from_eef(eef_pos, np.eye(3), adm_vel) # in the body frame

        Full_Jacobian = self.full_jacobian
        Jacobian = np.block([[eef_rotm.T, np.zeros((3,3))],[np.zeros((3,3)), eef_rotm.T]]) @ Full_Jacobian[:6, :6] # body Jacobian
        target_joint_vel = np.linalg.pinv(Jacobian) @ link6_vel # in the joint space
